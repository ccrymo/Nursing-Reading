...
import examRevision from "../../data/skills/skills_Reading";
...
const TitleSlide = React.memo(({ title }) => (
  <div className="flex items-center justify-center h-[calc(100vh-200px)]">
    <motion.h2
      ...
      className="text-4xl md:text-6xl lg:text-6xl font-bold text-amber-500 text-center px-6"
    >
        ...
        if (Array.isArray(value)) {
          return (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              className="bg-neutral-900/30 pb-20 px-6 rounded-xl backdrop-blur-sm"
              key={key}
            >
              <h3 className="text-2xl lg:text-4xl font-bold mb-8 text-amber-400">
        ...
        <div className="max-w-4xl mx-auto flex gap-4">
          ...
          <motion.button
            ...
            className={`flex-1 py-3 px-6 rounded-lg font-bold text-white hover:text-amber-950 ${
              isLastSlide ? "bg-neutral-600 cursor-not-allowed" : "bg-neutral-800 hover:bg-amber-500"
            } transition-colors`}
            onClick={handleNext}
